...
class BoatsForSalePageState extends State<BoatsForSalePage> {
  ...
  /// Indicates whether the user is currently in "create new boat" mode.
  ...
  Future<void> _initDatabase() async {
    ...
    boatDAO = db.boatDAO;
    // Load all existing boats from the database into memory.
  ...
  Future<void> _loadBoatsFromDatabase() async {
    // Retrieve all boats from the DAO.
  ...
  _ParsedBoatForm? _validateAndParseForm(BuildContext context) {
    ...
    if (price == null) {
      final msg = "InvalidPrice";
      formErrorMessage = msg;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg)));
      return null;
    }

    // Validate length as valid.
    if (length == null) {
      final msg = "InvalidLength";
      formErrorMessage = msg;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(msg)));
      return null;
    }
  ...
  /// 3. Inserts the boat using [boatDAO.insertBoat].
  ...
  Future<void> _submitNewBoat(BuildContext context) async {
    ...
    // Create a new Boat instance if user clicked add.
    final newBoat = Boat(
      null,
      parsed.yearBuilt,
      parsed.length,
  ...
  Future<void> _updateSelectedBoat(BuildContext context) async {
    ...
    // Apply the parsed form values to the currently selected boat.
    selectedBoat!.yearBuilt = parsed.yearBuilt;
    selectedBoat!.length = parsed.length;
    selectedBoat!.powerType = parsed.powerType;
    selectedBoat!.price = parsed.price;
    selectedBoat!.address = parsed.address;

    // Persist the updated boat in the database.
    await boatDAO.updateBoat(selectedBoat!);
  ...
  ///    - Deletes the boat using [BoatDAO.deleteBoat].
  ///    - Reloads the boat list.
  ...
  Future<void> _deleteSelectedBoat(BuildContext context) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: Text(
          "DeleteBoatConfirmTitle"
        ),
        content: Text(
          "DeleteBoatConfirmMessage"
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
  ...
  ///
  /// This is called when the user taps on a boat in the list to edit its details.
  void _populateFormFromBoat(Boat boat) {
    setState(() {
      // Copy all fields from the selected boat into the text controllers.
      yearController.text = boat.yearBuilt.toString();
      lengthController.text = boat.length.toString();
      powerController.text = boat.powerType;
  ...
  /// Builds a styled card representation for a single [boat] in the list.
  ///
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            "${boat.id}: ${boat.yearBuilt} ${boat.length} ${boat.powerType}",
  ...
  Widget ListPage() {
    return Padding(
      ...
      child: Column(
        children: [
          // Button to start creating a new boat.
          ElevatedButton(
            onPressed: () {
              setState(() {
          ...
          OutlinedButton(
            onPressed: () {
              showDialog(
                ...
                builder: (context) {
                  return AlertDialog(
                    title: Text("BoatInstructionsTitle"
                    ),
                    content: Text("BoatInstructions"                  ),
                    actions: [
                      ElevatedButton(
